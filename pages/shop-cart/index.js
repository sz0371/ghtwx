...
Page({
  ...
  onShow: function () {
    // 获取购物车数据
    wx.request({
      url: app.globalData.url + '/cart/getcartlist.json',
      method: 'POST',
      data: {
      ...
      success: (res) => {
        ...
        const shopList = res.data.data.resultList;
  ...
  delItem: function (e) {
    ...
    // 删除购物车商品
    wx.request({
      url: app.globalData.url + '/cart/removecart.json',
      method: 'POST',
      header: {
  ...
  totalPrice: function () {
    ...
    for (var i = 0; i < list.length; i++) {
      var curItem = list[i];
      if (curItem.active) {
        total += parseFloat(curItem.productPrice) * curItem.productNum;
      }
    }
  ...
  setGoodsList: function (saveHidden, total, allSelect, noSelect, list) {
    ...
    for (var i = 0; i < list.length; i++) {
      tempNumber = tempNumber + list[i].productNum
    }
  ...
  jiaBtnTap: function (e) {
    ...
    var list = this.data.goodsList.list;
    if (index !== "" && index != null) {
      if (list[parseInt(index)].productNum < 10) {
        list[parseInt(index)].productNum++;
        this.setGoodsList(this.getSaveHide(), this.totalPrice(), this.allSelect(), this.noSelect(), list);
      }
    }
  },
  jianBtnTap: function (e) {
    ...
    var list = this.data.goodsList.list;
    if (index !== "" && index != null) {
      if (list[parseInt(index)].productNum > 1) {
        list[parseInt(index)].productNum--;
        this.setGoodsList(this.getSaveHide(), this.totalPrice(), this.allSelect(), this.noSelect(), list);
      }
    }
